...
export class Constants {
    ...
    public static truffle_temp_dir = ""
    public static truffle_migrations_temp_dir = ""
    public static templates_directory = ""
    public static extensionName = "vetools"
    public static extensionVersion = "1.0.0"
    public static are_dependencies_installed = false

    public static default_host = "127.0.0.1"
    public static default_port = "8545"
    public static provider_url = `http://127.0.0.1:8545`
    ...
    public static truffle_config = {
        contracts_build_directory: "",
        contracts_directory: "",
        migrations_directory: "",
    }

    public static hardhat_config_default_directory = {
        contracts_build_directory: path.join("./", "artifacts", "contracts"),
        contracts_directory: path.join("./", "contracts"),
        scripts_directory: path.join("./", "scripts"),
    }
    ...
    public static ganacheRetryAttempts = 10
    ...
    public static requiredVersions: { [key: string]: { min: string; max: string } } = {
    ...
    public static initialize(context: ExtensionContext) {
        ...
        this.truffle_temp_dir = path.join(this.temporaryDirectory, "thor")
        if (!fs.existsSync(this.truffle_temp_dir)) {
            fs.mkdirSync(this.truffle_temp_dir)
        }

        this.truffle_migrations_temp_dir = path.join(this.truffle_temp_dir, "migrations")
        if (!fs.existsSync(this.truffle_migrations_temp_dir)) {
            fs.mkdirSync(this.truffle_migrations_temp_dir)
        }

        this.templates_directory = path.resolve(__dirname, "../../", "resources", "templates")

        this.truffle_config = {
            contracts_build_directory: path.join(this.truffle_temp_dir, "build", "contracts"),
            contracts_directory: path.join(this.truffle_temp_dir, "contracts"),
            migrations_directory: path.join(this.truffle_temp_dir, "migrations"),
        }

        console.debug("truffle build dir:", this.truffle_config.contracts_build_directory)
        ...
        this.treeItemData.project.bdm.iconPath = {
            ...
            light: context.asAbsolutePath(
                path.join("resources/light", "BlockchainDataManager-service_and_project.svg")
            ),
        }
