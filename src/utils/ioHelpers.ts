...
export default class IoHelpers {
    static async choosePassword(prompt: string, acceptEmptyString?: boolean): Promise<string | undefined> {
        const input = await vscode.window.showInputBox({ prompt, password: true })
        if (input === "") {
            if (acceptEmptyString) {
                if (await IoHelpers.yesNo("Are you sure that you want to use an empty password?")) {
                    return input
                } else {
        ...
        // showWarningMessage is not await'ed so that the password prompt appears at the same time:
        vscode.window.showWarningMessage("The two passwords that you entered did not match.")
    ...
    static async enterNumber(
        ...
        const input = await vscode.window.showInputBox({
            prompt,
            validateInput: (_) =>
                isNaN(parseFloat(_)) ? "Enter a numeric value" : additionalValidation(parseFloat(_)),
    ...
    static async enterString(prompt: string, value?: string): Promise<string | undefined> {
    ...
    static async pickFolder(defaultUri?: vscode.Uri): Promise<string | undefined> {
