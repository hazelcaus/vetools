...
class InfuraClient {
    ...
    private readonly showProjectsFromInfuraCommand = "vetools.showProjectsFromInfuraAccount"
    ...
    public getExcludedProjects(): IInfuraProjectDto[] {
        if (this.globalState) {
            return (
                this.globalState.get<IInfuraProjectDto[]>(Constants.globalStateKeys.infuraExcludedProjectsListKey) || []
            )
        } else {
            return []
        }
    ...
    public async getAllowedProjects(): Promise<IInfuraProjectDto[]> {
        ...
        return allProjects.filter(
            (project) => !this.getExcludedProjects().some((excluded) => excluded.id === project.id)
        )
    }
    ...
    public async getProjectDetails(projectId: string): Promise<IInfuraProjectDto> {
        const url = new URL(
            `${Constants.infuraAPIUrls.projects}/${projectId}`,
            Constants.infuraAPIUrls.rootURL
        ).toString()
        const params = {
            method: "GET",
            url,
