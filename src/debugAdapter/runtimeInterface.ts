...
import { filter_contracts_with_addresses, prepare_contracts } from "./contracts/contractsPrepareHelpers"
...
export default class RuntimeInterface extends EventEmitter {
    ...
    public callStack(): ICallInfo[] {
        ...
        for (let i = 0; i < callStack.length - 1; i++) {
            ...
            if (contract === undefined) {
                throw new Error(
                    `Coundn\'t find contract by address ${callStack[i].address || callStack[i].storageAddress}`
                )
            }
    ...
    public async attach(txHash: string, working_directory: string): Promise<void> {
        ...
        this._deployedContracts = filter_contracts_with_addresses(result.contracts)
        ...
        console.debug("[attach] options.files:", result.files)
    ...
    public currentLine(): DebuggerTypes.IFrame {
        ...
        if (!sourcePath) {
            console.debug("We are here since we couldn't find any sourcePath:", sourcePath)
            throw new Error("Couldn't find any source file")
        }
    ...
    public send_event(event: string, ...args: any[]) {
        setImmediate(() => {
