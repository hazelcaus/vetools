            ...
            switch (variable.type.typeClass) {
                ...
                case "bytes":
                    switch (variable.type.kind) {
                        case "static":
                            return createResult(
                                variable,
                                (<BytesStaticValue>variable).value.asHex,
                        ...
                        default:
                            return createResult(
                                variable,
                                (<BytesValue>variable).value.asHex,
                                (<BytesValue>variable).type.typeHint
                            )
                    }
                case "address":
                ...
                case "string": {
                    switch ((<StringValue>variable).value.kind) {
                        case "valid":
                            return createResult(
                                variable,
                                (<StringValueInfoValid>variable.value).asString,
                                variable.type.typeHint
                            )
                        case "malformed":
                            ...
                            return createResult(
                                variable,
                                Buffer.from(
                                    (<StringValueInfoMalformed>variable.value).asHex.slice(2),
                                    "hex"
                                ).toString(),
                                variable.type.typeHint
                            )
                ...
                case "mapping":
                    ...
                    return createResult(
                        variable,
                        new Map(
                            (<Format.Values.MappingValue>variable).value.map(({ key, value }) => [
                                translate(key),
                                translate(value),
                            ])
                        )
                ...
                case "userDefinedValueType": {
                    ...
                    return createResult(
                        variable,
                        `${typeName}.wrap(${inspectOfUnderlying})`,
                        coercedResult.type.typeName
                    ) //note only the underlying part is stylized
                }
                case "tuple": {
                        ...
                        case "internal": {
                            ...
                            switch (coercedResult.value.kind) {
                                ...
                                case "unknown":
                                    ...
                                    const breakingSpace =
                                        firstLine.length + secondLine.length + 1 > breaklength ? "\n" : " "
                                    //now, put it together
                                    return createResult(variable, firstLine + breakingSpace + secondLine)
                                default:
